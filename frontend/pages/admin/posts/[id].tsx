...
export default function EditPostPage() {
  ...
  const loadPost = async () => {
    try {
      ...
      const response = await fetch(`${base}/api/proxy/api/posts/all?page=1&page_size=1000`, {
      ...
      const foundPost = data.results?.find((p: Post) => p.id === parseInt(id as string));
      ...
      setStatus(foundPost.status as 'draft' | 'published');
    } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const base = process.env.NEXT_PUBLIC_BASE_URL || '';
      const response = await fetch(`${base}/api/proxy/api/posts/${id}`, {
        method: 'PUT',
        headers: {
      ...
      setToast({ message: err.message || 'Failed to update post', type: 'error' });
    } finally {
      setSubmitting(false);
    }
  };
