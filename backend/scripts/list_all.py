...
def print_staging(s):
    ...
    for r in rows:
        print(
            f"[STAGING {r.id}] {r.name} | status={r.status} | website={r.website} | address={r.address} | lat={r.latitude} lon={r.longitude}"
        )
...
def print_schools(s):
    ...
    for r in rows:
        print(
            f"[SCHOOL {r.id}] {r.name} | status={r.status} | website={r.website} | address={r.address} | lat={r.latitude} lon={r.longitude}"
        )
